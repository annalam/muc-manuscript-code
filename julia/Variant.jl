...
VCF() = VCF([], [], [], [], [], [], [], zeros(0, 0), zeros(0, 0),
	falses(0, 0), zeros(0, 0), zeros(0, 0), zeros(0, 0))
Base.getindex(vcf::VCF, rows, cols) = VCF(vcf.sample[cols],
	vcf.chromosome[rows], vcf.position[rows], vcf.ref_allele[rows],
	vcf.alt_allele[rows], vcf.gene[rows], vcf.effect[rows],
	vcf.alt[rows, cols], vcf.total[rows, cols], vcf.star[rows, cols], vcf.baseq[rows, cols], vcf.mapq[rows, cols], vcf.sidedness[rows, cols])
...
is_truncating(effect) = r"Frameshift|Stopgain|Stoploss|Splice" in effect
is_coding_region(effect) = r"Synonymous|Missense|[Ff]rameshift|Stopgain|Stoploss|Splice" in effect
...
read_vcf(vcf_cmd::Base.AbstractCmd) = read_vcf(open(vcf_cmd))
